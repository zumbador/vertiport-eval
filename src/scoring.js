...
export function pointInPolygon(lat, lon, verts) {
  let inside = false;
  for (let i = 0, j = verts.length - 1; i < verts.length; j = i++) {
    const xi = verts[i][1], yi = verts[i][0], xj = verts[j][1], yj = verts[j][0];
    if ((yi > lat) !== (yj > lat) && lon < ((xj - xi) * (lat - yi)) / (yj - yi) + xi) inside = !inside;
  }
  return inside;
}

export function polyCenter(verts) {
  return [verts.reduce((s, v) => s + v[0], 0) / verts.length, verts.reduce((s, v) => s + v[1], 0) / verts.length];
}

export function polyRadius(center, verts) {
  return Math.max(...verts.map(v => _hav(center[0], center[1], v[0], v[1])));
}

// ── Regrid area parcel fetch (radius in metres → converted to feet) ─
export async function fetchRegridAreaParcels(lat, lon, radiusM, regridKey, limit = 200) {
  const radiusFt = Math.ceil(radiusM * 3.28084);
  const url = `https://app.regrid.com/api/v2/point?lat=${lat}&lon=${lon}&radius=${radiusFt}&limit=${limit}&token=${encodeURIComponent(regridKey)}`;
  const res = await fetch(url, { signal: AbortSignal.timeout(30000) });
  if (!res.ok) throw new Error(`Regrid area API ${res.status}`);
  const json = await res.json();
  return json?.parcels?.features ?? [];
}

// Pre-screen + fast-score a Regrid parcel — returns null if screened out
export function quickScoreParcel(fields, lat, lon) {
  const acreage = parseFloat(fields.ll_gisacre) || 0;
  if (acreage < 0.3) return null;
  const ud = (fields.usedesc || "").toLowerCase();
  if (/^(single.family|residential|sfr|condo|townhome|townhouse|duplex|mobile|multifamily|apartment)/.test(ud)) return null;
  const { score: parcelScore, flags } = scoreParcelAcreage(acreage, "Regrid");
  const air = scoreAirspace(lat, lon);
  const siteEst = Math.min(100, Math.round(parcelScore * 0.40 + air.score * 0.40 + 50 * 0.20));
  return {
    parcelScore, airScore: air.score, airStatus: air.status, siteEst, acreage,
    usedesc: fields.usedesc || "—", address: fields.address || "",
    parcelnumb: fields.parcelnumb || "", lat, lon, flags,
  };
}

// ── Regional heatmap helpers ──────────────────────────────────────

export async function fetchHeatmapPois(verts) {
  const polyStr = verts.map(v => `${v[0]} ${v[1]}`).join(" ");
  const q = `[out:json][timeout:25];(node["amenity"](poly:"${polyStr}");node["shop"](poly:"${polyStr}");node["office"](poly:"${polyStr}");node["healthcare"](poly:"${polyStr}"););out;`;
  const res = await fetch("https://overpass-api.de/api/interpreter", {
    method: "POST", body: q, signal: AbortSignal.timeout(25000),
  });
  if (!res.ok) throw new Error(`Overpass ${res.status}`);
  const json = await res.json();
  return (json.elements || []).map(e => ({ lat: e.lat, lon: e.lon }));
}

export function buildHeatmapGrid(verts, pois, gridSize = 7) {
  const lats = verts.map(v => v[0]), lons = verts.map(v => v[1]);
  const minLat = Math.min(...lats), maxLat = Math.max(...lats);
  const minLon = Math.min(...lons), maxLon = Math.max(...lons);
  const dLat = (maxLat - minLat) / gridSize, dLon = (maxLon - minLon) / gridSize;

  const heatColor = (s) => s >= 75 ? "#1a8a58" : s >= 55 ? "#5B9BD5" : s >= 40 ? "#c87a10" : "#C0392B";

  const cells = [];
  for (let r = 0; r < gridSize; r++) {
    for (let c = 0; c < gridSize; c++) {
      const cLat = minLat + (r + 0.5) * dLat, cLon = minLon + (c + 0.5) * dLon;
      if (!pointInPolygon(cLat, cLon, verts)) continue;
      const air = scoreAirspace(cLat, cLon);
      const poiCount = pois.filter(p =>
        p.lat >= minLat + r * dLat && p.lat < minLat + (r + 1) * dLat &&
        p.lon >= minLon + c * dLon && p.lon < minLon + (c + 1) * dLon
      ).length;
      const poiScore = Math.min(100, poiCount * 5);
      const score = Math.round(air.score * 0.5 + poiScore * 0.5);
      cells.push({
        bounds: [[minLat + r * dLat, minLon + c * dLon], [minLat + (r + 1) * dLat, minLon + (c + 1) * dLon]],
        score, airScore: air.score, poiScore, poiCount, airStatus: air.status,
        color: heatColor(score), lat: cLat, lon: cLon,
      });
    }
  }
  return cells;
